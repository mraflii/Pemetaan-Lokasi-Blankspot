    ...
    if($kota_list->num_rows > 0) {
        ...
        // Ambil desa untuk semua kecamatan
        if(count($kecamatan_data) > 0) {
            $kecamatan_kodes = array_column($kecamatan_data, 'kode_wilayah');
            $kecamatan_ids = implode("','", $kecamatan_kodes);
            
            $desa_query = $conn->query("
                SELECT d.*, kec.nama as kecamatan_nama, kota.nama as kota_nama 
                FROM wilayah d 
                JOIN wilayah kec ON d.parent_kode = kec.kode_wilayah 
                JOIN wilayah kota ON kec.parent_kode = kota.kode_wilayah 
                WHERE d.parent_kode IN ('$kecamatan_ids') AND d.level='desa' 
                ORDER BY kota.nama, kec.nama, d.nama ASC
            ");
            
            while($desa = $desa_query->fetch_assoc()) {
                $desa_data[] = $desa;
            }
        }
    }
    ...
    * {
    ...
    .level-option.lokasi .level-icon { color: var(--danger); }
    ...
    <body>
            ...
            <div class="form-container fade-in">
                ...
                <div class="level-selection">
                    ...
                    <div class="level-options">
                        ...
                        <div class="level-option lokasi" onclick="selectLevel('lokasi')">
                            <div class="level-icon">
                                <i class="fas fa-map-marker-alt"></i>
                            </div>
                            <div class="level-name">Lokasi</div>
                            <div class="level-desc">Tambahkan lokasi langsung ke desa tertentu</div>
                        </div>
                    </div>
                </div>

                <!-- Parent Selection (untuk kecamatan, desa, dan lokasi) -->
                ...
                <form action="proses/proses_add_wilayah_sekaligus.php" method="post" id="formBulk">
                    ...
                    <!-- Lokasi Section -->
                    <div class="form-section" id="lokasiSection">
                        <div class="section-header">
                            <div class="section-title">
                                <i class="fas fa-map-marker-alt"></i>
                                <span>Tambahkan Lokasi Baru</span>
                            </div>
                            <div class="counter-badge" id="lokasiCounter">0 lokasi</div>
                        </div>
                        <div class="section-content" id="lokasiWrapper">
                            <!-- Lokasi template akan ditambahkan di sini -->
                        </div>
                    </div>
        ...
        <script>
            ...
            let desaList = <?= json_encode($desa_data) ?>;
            let formCounter = {
                kota: 0,
                kecamatan: 0,
                desa: 0,
            ...
            function selectLevel(level) {
                ...
                // Tampilkan parent selection untuk kecamatan, desa, dan lokasi
                if (level === 'kecamatan' || level === 'desa' || level === 'lokasi') {
            ...
            function showParentSelection(level) {
                ...
                if (level === 'kecamatan') {
                    ...
                    parentSteps.innerHTML = `
                        ...
                        <div class="step" id="stepDesa">
                            <div class="step-title">3. Pilih Desa</div>
            ...
            // Render options desa berdasarkan kecamatan
            function renderDesaOptions(kecamatanKode) {
                const desaInKecamatan = desaList.filter(desa => desa.parent_kode === kecamatanKode);
                
                if (desaInKecamatan.length === 0) {
                    return `
                        <div class="empty-state">
                            <i class="fas fa-home"></i>
                            <div>Belum ada desa di kecamatan ini.</div>
                        </div>
                    `;
                }
                
                let html = '<div class="parent-options">';
                desaInKecamatan.forEach(desa => {
                    html += `
                        <div class="parent-option" onclick="selectDesa('${desa.kode_wilayah}', '${desa.nama.replace(/'/g, "\\'")}')">
                            ${desa.nama}
                        </div>
                    `;
                });
            ...
            function selectKecamatan(kode, nama) {
                ...
                if (selectedLevel === 'desa') {
                    ...
                    document.getElementById('desaOptions').innerHTML = renderDesaOptions(kode);
                }
            }
            ...
            function selectDesa(kode, nama) {
                ...
                // Tampilkan form lokasi
                showSelectedParentInfo(`${selectedKota.nama} → ${selectedKecamatan.nama} → ${nama} (Desa)`);
                showFormSection('lokasi');
            }
            ...
            function showSelectedParentInfo(text) {
                ...
                let parentKode = '';
                if (selectedLevel === 'kecamatan') {
                    parentKode = selectedKota.kode;
                } else if (selectedLevel === 'desa') {
                    parentKode = selectedKecamatan.kode;
                } else if (selectedLevel === 'lokasi') {
                    parentKode = selectedDesa.kode;
                }
            ...
            function addKotaForm(kotaData = null) {
                ...
                kotaDiv.innerHTML = `
                    ...
                    <div class="action-buttons">
                        <button type="button" class="btn btn-success btn-sm" onclick="addKotaForm()">
                            <i class="fas fa-plus"></i> Tambah Kota Lain
                        </button>
                    </div>
                `;
            ...
            function addKecamatanForKota(kotaIndex, kecamatanData = null) {
                ...
                kecamatanDiv.innerHTML = `
                    ...
                    <div class="action-buttons">
                        <button type="button" class="btn btn-success btn-sm" onclick="addKecamatanForKota(${kotaIndex})">
                        ...
                        <div class="form-group">
                            ...
                            <div class="coordinate-input-group">
                                <input type="text" 
                                    ...
                                    oninput="handleCoordinateInput('${fullDesaKode}', ${lokasiIndex}, this.value, 'kota')"
                                    required>
                                <button type="button" class="btn btn-primary btn-sm" onclick="openMapModal('${fullDesaKode}', ${lokasiIndex}, 'kota')">
                        ...
                        <div class="form-group">
                            ...
                            <div class="coordinate-input-group">
                                <input type="text" 
                                    ...
                                    placeholder="Latitude,Longitude"
                                    value="${lokasiData ? lokasiData.koordinat : ''}"
                                    oninput="handleCoordinateInput('${fullDesaKode}', ${lokasiIndex}, this.value, 'standalone')"
                                    required>
                        ...
                        <div class="form-group">
                            ...
                            <div class="coordinate-input-group">
                                <input type="text" 
                                    ...
                                    placeholder="Latitude,Longitude"
                                    value="${lokasiData ? lokasiData.koordinat : ''}"
                                    oninput="handleCoordinateInput('${fullDesaKode}', ${lokasiIndex}, this.value, 'only')"
                                    required>
            ...
            // Tambah form lokasi (standalone)
            function addLokasiForm(lokasiData = null) {
                const wrapper = document.getElementById('lokasiWrapper');
                const index = formCounter.lokasi++;
                ...
                lokasiDiv.innerHTML = `
                    ...
                    <div class="lokasi-form">
                        <h4><i class="fas fa-map-marker-alt"></i> Data Lokasi <span class="lokasi-counter">#${index + 1}</span></h4>
                        
                        <div class="form-group">
                            <label for="lokasi_direct_nama_${index}"><i class="fas fa-tag"></i> Nama Tempat *</label>
                            <input type="text" 
                                class="nama-field" 
                                id="lokasi_direct_nama_${index}" 
                                name="lokasi[${index}][nama_tempat]" 
                                placeholder="Masukkan nama tempat"
                                value="${lokasiData ? lokasiData.nama_tempat : ''}"
                                required>
                        </div>
                        ...
                        <div class="form-group">
                            <label for="lokasi_direct_koordinat_${index}"><i class="fas fa-map-pin"></i> Koordinat *</label>
                            <div class="coordinate-input-group">
                                <input type="text" 
                                    class="kode-field" 
                                    id="lokasi_direct_koordinat_${index}" 
                                    name="lokasi[${index}][koordinat]" 
                                    placeholder="Latitude,Longitude"
                                    value="${lokasiData ? lokasiData.koordinat : ''}"
                                    oninput="handleCoordinateInput('${selectedDesa.kode}', ${index}, this.value, 'direct')"
                                    required>
                                <button type="button" class="btn btn-primary btn-sm" onclick="openMapModal('${selectedDesa.kode}', ${index}, 'direct')">
                                    <i class="fas fa-map"></i> Pilih di Peta
                                </button>
                            </div>
                            <div class="coordinate-display" id="lokasi_direct_coordinate_display_${index}">
                                ${lokasiData ? lokasiData.koordinat : 'Klik "Pilih di Peta" untuk menentukan koordinat'}
                            </div>
                        </div>
                        
                        <div class="form-group">
                            <label for="lokasi_direct_keterangan_${index}"><i class="fas fa-file-text"></i> Keterangan</label>
                            <textarea class="nama-field" 
                                    id="lokasi_direct_keterangan_${index}" 
                                    name="lokasi[${index}][keterangan]" 
                                    placeholder="Tambahkan keterangan tentang lokasi ini (opsional)"
                                    rows="2">${lokasiData ? lokasiData.keterangan : ''}</textarea>
                        </div>
                        
                        <div class="form-group">
                            <label for="lokasi_direct_sinyal_${index}"><i class="fas fa-wifi"></i> Ketersediaan Sinyal *</label>
                            <select class="select-field" 
                                    id="lokasi_direct_sinyal_${index}" 
                                    name="lokasi[${index}][ketersediaan_sinyal]"
                                    required>
                                <option value="Yes" ${lokasiData && lokasiData.ketersediaan_sinyal === 'Yes' ? 'selected' : ''}>Yes - Ada Sinyal</option>
                                <option value="No" ${lokasiData && lokasiData.ketersediaan_sinyal === 'No' ? 'selected' : ''}>No - Tidak Ada Sinyal</option>
                            </select>
                            <div class="signal-status">
                                <span class="status-indicator status-${lokasiData && lokasiData.ketersediaan_sinyal === 'Yes' ? 'yes' : 'no'}"></span>
                                <span>Status: <strong id="lokasi_direct_status_text_${index}">${lokasiData && lokasiData.ketersediaan_sinyal === 'Yes' ? 'Ada Sinyal' : 'Tidak Ada Sinyal'}</strong></span>
                            </div>
                        </div>
                        
                        <div class="form-group">
                            <label for="lokasi_direct_kecepatan_${index}"><i class="fas fa-tachometer-alt"></i> Kecepatan Sinyal (Mbps)</label>
                            <input type="number" 
                                class="kode-field" 
                                id="lokasi_direct_kecepatan_${index}" 
                                name="lokasi[${index}][kecepatan_sinyal]" 
                                value="${lokasiData ? lokasiData.kecepatan_sinyal : '0'}"
                                min="0" 
                                step="0.1"
                                placeholder="Contoh: 10.5">
                        ...
                        <input type="hidden" 
                            name="lokasi[${index}][kode_wilayah]" 
                            value="${selectedDesa.kode}">
                    </div>
                    
                    <div class="action-buttons">
                        <button type="button" class="btn btn-primary btn-sm" onclick="addLokasiForm()">
                            <i class="fas fa-plus"></i> Tambah Lokasi Lain
                        </button>
                    </div>
                `;
                ...
                // Add event listener for signal status change
                document.getElementById(`lokasi_direct_sinyal_${index}`).addEventListener('change', function(e) {
                    const statusIndicator = document.querySelector(`#lokasi_direct_status_text_${index}`).previousElementSibling;
                    const statusText = document.getElementById(`lokasi_direct_status_text_${index}`);
            ...
            function updateCounters() {
                ...
                const lokasiCount = document.querySelectorAll('#lokasiWrapper .wilayah-group').length;
                ...
                document.getElementById('lokasiCounter').textContent = `${lokasiCount} lokasi`;
            }
            ...
            function openMapModal(kodeWilayah, lokasiIndex, type = 'kota') {
                ...
                if (type === 'kota') {
                    ...
                    inputId = `lokasi_direct_koordinat_${lokasiIndex}`;
                }
            ...
            function saveCoordinates() {
                ...
                if (type === 'kota') {
                    ...
                    inputId = `lokasi_direct_koordinat_${lokasiIndex}`;
                    displayId = `lokasi_direct_coordinate_display_${lokasiIndex}`;
                }
            ...
            function handleCoordinateInput(kodeWilayah, lokasiIndex, coordinateValue, type) {
                // Update display
                let displayId;
                if (type === 'kota') {
                    displayId = `lokasi_coordinate_display_${lokasiIndex}`;
                ...
                document.getElementById(displayId).textContent = coordinateValue;
                ...
                if (isValidCoordinate(coordinateValue)) {
                    ...
                    if (document.getElementById('mapModal').style.display !== 'flex') {
                        openMapModal(kodeWilayah, lokasiIndex, type);
                    }
            ...
            document.getElementById('formBulk').addEventListener('submit', function(e) {
                ...
                if (selectedLevel === 'kota') {
                    ...
                    count = document.querySelectorAll('#lokasiWrapper .wilayah-group').length;
                }
